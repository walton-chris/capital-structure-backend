...
# --- App (start with NO external deps to avoid import-time crashes) ---
app = FastAPI(
...
# Allow explicit prod domain + any Vercel preview https://*.vercel.app
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
        "http://localhost:5173",
...
# --- Lazy Redis/RQ getters to avoid import-time failures ---
_redis = None
_q = None
...
def get_redis():
    ...
    if not url:
        # Do NOT crash app init; raise later in endpoints that require Redis
...
# In-memory file registry for this web process
FILES: Dict[str, Dict[str, Any]] = {}

# --- Extra safety: catch-all OPTIONS so preflight never hits user code paths ---
@app.options("/{full_path:path}")
async def preflight(full_path: str, request: Request):
    # CORSMiddleware will add the proper headers; we just return 204 fast.
...
async def health():
    ...
    try:
        r = get_redis()  # may raise if not configured
        r.ping()
    except Exception as e:
        # Report degraded, but DO NOT 503—platform health checks often expect 200
...
async def upload_document(req: FileUploadRequest):
    # No Redis dependency here—keep uploads alive even if Redis is down
    try:
...
async def submit_extract(payload: DocumentExtractRequest):
    # Require Redis/RQ here (but only here)
    try:
        r = get_redis()
        q = get_queue()
    except Exception as e:
...
async def get_job_status(job_id: str):
    # Lazy import only here to avoid import-time failures
    try:
